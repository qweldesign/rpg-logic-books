...
RPG.LOGIC では、主に3つの判定を用います。  
通常の判定 (攻撃判定、防御判定などの技能判定)、成功度 (失敗度) 判定、ダメージ判定の3つです。  
まず、単にダイスを転がして出目の合計を得る `function getRoll()` と `type roll` から定義していきましょう。  
それに「成功/失敗」と「クリティカル」の有無をプロパティとして取る `function judge` と `type Judge` を定義し、さらに「成功度/失敗度」をプロパティとして取る `function score` と `type Score` を定義すれば、ダイスによる判定の実装は完了です。  
ダメージ判定は、後の `Action/resolver` にて定義します。
