...
`nextTurn()` メソッドで、フェイント効果は毎ターンリセットするようにします。  
フェイント効果はフェイントを行った次ターンに有効なので、`true` / `false` だけでなく、`null` の状態も用いて管理します。  
フェイントを行ったターンは変数を `true` にし、次の適用されるターンに `false` にし、そのターンの終わりに フェイント・オブジェクト自体を `null` にします。
