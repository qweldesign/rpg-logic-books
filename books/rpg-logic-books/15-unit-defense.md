...
`nextTurn()` メソッドで、「受け」「止め」の試行回数、全力攻撃、全力防御は毎ターンリセットするようにします。  
全力攻撃、全力防御はそのターンの終わりまで有効なので、`boolean` 型変数を2つ用いて管理します。  
そのターンに全力攻撃、全力防御を選択したなら、それぞれを選択したターンのフラグの変数を `true` に変更し、そのターンの終わりに効果適用フラグの変数を `true` に変え、選択したターンのフラグの変数は `false` に戻しておきます。次のターンの終わりに、効果適用フラグの変数は `false` に戻します。
